...
export function buildFieldMapping(
  ...
  for (const [fieldName, def] of Object.entries(fields)) {
    if (def && typeof def === "object" && typeof def.tn_role === "string") {
      ...
      if (roleToField[role] !== undefined) continue;
 ...
 * Accepts collectionPath directly (no CLI config resolution).
 */
export async function loadFieldMapping(collectionPath: string): Promise<FieldMapping> {
  try {
...
/**
 * Resolve the display title for a task.
 */
...
/**
 * Get status enum values from the type definition.
 */
export async function getStatusValues(collectionPath: string): Promise<string[]> {
  try {
    const configResult = await loadConfig(collectionPath);
    if (!configResult.valid || !configResult.config) return [];
    const typeResult = await getType(collectionPath, configResult.config, "task");
    if (!typeResult.valid || !typeResult.type) return [];
...
/**
 * Get priority enum values from the type definition.
 */
export async function getPriorityValues(collectionPath: string): Promise<string[]> {
  try {
    const configResult = await loadConfig(collectionPath);
    if (!configResult.valid || !configResult.config) return [];
    const typeResult = await getType(collectionPath, configResult.config, "task");
    ...
    if (priorityField?.values && Array.isArray(priorityField.values)) {
